...
# Esp8266 Arduino git location. Change ESP_ROOT to match your path
ESP_ROOT ?= $(HOME)/esp8266/
# Output directory
BUILD_ROOT ?= /tmp/$(MAIN_NAME)

# Board definitions. Assumes 4M flash, 1M SPIFF
FLASH_SIZE ?= 4M
FLASH_MODE ?= dio
FLASH_SPEED ?= 40
FLASH_LAYOUT ?= eagle.flash.4m.ld

# Upload parameters. Change UPLOAD_PORT to match your port name
UPLOAD_SPEED ?= 230400
